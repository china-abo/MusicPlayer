...
var start = function() {
    $('.play').on('click',function(){
        ...
        if(audio.paused == true){
            ...
            $('.cd').css('animation', '10s transform linear infinite')
            play()
        } else{
...
var volicTogg = function() {
    $('.volume').on('click', function() {
        ...
        if(audio.muted === false){
            audio.muted = true
            $('.volume').css('background-image', 'url(img/Volume_Off.png)')
        } else {
            audio.muted = false
            $('.volume').css('background-image', 'url(img/Volume_On.png)')
        }
    })
}
//menu开关
var menu = function() {
    $('.menu').on('click', function() {
        $('.list').toggleClass('list-none')
        $('.name').toggleClass('block')
        $('.title').toggleClass('block')
    })
}
//跳跃加载
var jump = function() {
...
var list = function() {
    for (var i = 0; i < string.length; i++) {
        var t = `
        <p class='name' data-id='${string[i].id}' data-path='music/${string[i].name}-${string[i].author}.mp3'>${string[i].name}-${string[i].author}</p>
        `
        $('.music-name').append(t)
    }
}
...
var SongPlay = function() {
    $('.music-name').on('click', '.name', function(event) {
        var t = $(event.target)
        //给播放歌曲添加状态
        $('.active').removeClass('active')
        t.addClass('active')
...
var setTime = function() {
    ...
    $('.play-time').text(timeLeft)
    $('.total-time').text(timeRight)
}
